...
public class GamePrinter {
	...
	protected String getInfo() {
		...
		/* @formatter:off */
		if(game.getCycle() == 0) {
			buffer.append(Messages.CONFIGURED_LEVEL.formatted(game.getLevel())).append(NEW_LINE);
			buffer.append(Messages.CONFIGURED_SEED.formatted(game.getSeed())).append(NEW_LINE);
		}
		...
		buffer.append(Messages.SCORE).append(SPACE).append(game.getScore()).append(NEW_LINE);
		/* @formatter:on */
	...
	public String endMessage(){
		...
		if (game.isPlayerDied())
			buffer.append(Messages.GAME_OVER).append(NEW_LINE).append(Messages.ZOMBIES_WIN);
		else if(game.allZombiesDied())
			buffer.append(Messages.GAME_OVER).append(NEW_LINE).append(Messages.PLAYER_WINS);
		else if(game.isPlayerQuits())
			buffer.append(Messages.GAME_OVER).append(NEW_LINE).append(Messages.PLAYER_QUITS);
		
		buffer.append(NEW_LINE);
		
		if(game.getRecord() >= game.getScore())
			buffer.append(Messages.RECORD).append(game.getRecord());
		else
			buffer.append(Messages.NEW_RECORD).append(game.getScore());
